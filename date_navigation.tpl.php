 ...
 */
	...
	$prev_url = date("Ym", $active_date) > date("Ym") ? "events/".date("Y-m", strtotime($view_arg." -1 month")) : '';
	$next_url = "events/".date("Y-m", strtotime($view_arg." +1 month"));
?>
<div class="date-nav clear-block">
  <div class="date-prev">
    <?php if (!empty($prev_url)) print l('<', $prev_url, $prev_options); else print "&nbsp;"; ?>
  </div>
  <div class="date-heading">
    <?php print str_replace(date("/Y-"), date("/Y-", $active_date), $nav_title); ?>
  </div>
  <div class="date-next">
    <?php if (!empty($next_url)) print l('>', $next_url, $next_options); else print "&nbsp;"; ?>
  </div>
</div>
